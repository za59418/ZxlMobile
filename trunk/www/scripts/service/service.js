...
.factory('jobService', ['localStorageService', 'dateService',
        ...
        return {
            init: function (projects) {
                ...
                if (projects) {
                    ...
                    for (; i < length; i++) {
                        messageDate = dateService.getMessageDate(projects[i]);
                        if (!messageDate) {
                            return null;
                        }
                        date = new Date(messageDate.year, messageDate.month,
                            ...
                            messageDate.second);
                        projects[i].lastMessage.timeFrome1970 = date.getTime();
                        projectID[i] = {
                            id: projects[i].id
                        };

                    }
                    localStorageService.update("projectID", projectID);
                    for (i = 0; i < length; i++) {
                        localStorageService.update("project_" + projects[i].id, projects[i]);
                    }
                }
            },
            getAllProjects: function () {
                ...
                if (projectID) {
                    ...
                    for (; i < length; i++) {
                        project = localStorageService.get("project_" + projectID[i].id);
                        if (project) {
                            projects.push(project);
                        }
                    }
                    dateService.handleMessageDate(projects);
                    return projects;
                }
                return null;
...
.factory('applicationService', ['localStorageService',
    function (localStorageService) {
        return {
            init: function (applications) {
                ...
                localStorageService.update("applicationID", applicationID);
                for (i = 0; i < length; i++) {
                    localStorageService.update("application_" + applications[i].id, applications[i]);
                }
            },
            getAllApplications: function ()
            {
                var apps = new Array();
                var i = 0;
                var applicationID = localStorageService.get("applicationID");
                var length = 0;
                var app = null;
                if (applicationID) {
                    length = applicationID.length;

                    for (; i < length; i++) {
                        app = localStorageService.get("application_" + applicationID[i].id);
                        if (app) {
                            apps.push(app);
                        }
                    }
...
.factory('userService', ['localStorageService', function (localStorageService) {
    return {
        init: function (userInfo) {
            localStorageService.update("userID", userInfo);
        },
